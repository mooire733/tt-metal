relu_tile
=========

.. doxygenfunction:: relu_tile_init()
.. doxygenfunction:: relu_tile(uint32_t idst)
.. doxygenfunction:: relu_tile_int32(uint32_t idst)

relu_max_tile
=============

.. doxygenfunction:: relu_max_tile_init()
.. doxygenfunction:: relu_max_tile(uint32_t idst, uint32_t upper_limit)
.. doxygenfunction:: relu_max_tile_int32(uint32_t idst, uint32_t param0)


relu_min_tile
=============

.. doxygenfunction:: relu_min_tile_init()
.. doxygenfunction:: relu_min_tile(uint32_t idst, uint32_t lower_limit)
.. doxygenfunction:: relu_min_tile_int32(uint32_t idst, uint32_t param0)


leaky_relu_tile
===============

.. doxygenfunction:: leaky_relu_tile_init()
.. doxygenfunction:: leaky_relu_tile(uint32_t idst, uint32_t lower_limit)

Compiler-generated BF16 integration
-----------------------------------

.. doxygenfunction:: relu_tt_poly_bf16_tile_init
.. doxygenfunction:: relu_tt_poly_bf16_tile
