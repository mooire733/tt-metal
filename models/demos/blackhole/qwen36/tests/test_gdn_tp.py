...
def test_gdn_tp_prefill_fused_vs_phased_bit_exact(mesh_device, monkeypatch, T, reset_seeds, ensure_gc):
    ...
    T=2048 is the production chunk length; T=256 keeps a cheap short case. Both need an out-projection
    that survives a long prefill, which is why this gate waited for the column-parallel one (#57377,
    now on main) and its persistent all-gather buffer (#57458). The row-parallel arm it replaced
    cannot serve this test at all: its fused reduce-scatter FATALs above ~273 tokens per chunk
    (#57519), so the layer could not even run at T=2048 on that path."""
    os.environ.setdefault("HF_MODEL", model_path())
