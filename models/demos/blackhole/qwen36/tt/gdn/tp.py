...
class TPGatedDeltaNet:
    ...
    def _conv1d_prefill(self, qkv, T, conv_state, _force_splice=False):
        ...
        Lin = (K - 1) + T
        # From-scratch chunks let ttnn.conv1d apply the K-1 causal pad instead of a 197us concat.
        _splice = (
        ...
        # Carry splice: bit-exact but OFF (_SPLICE_CARRY). Only rows t < K-1 see the carry, so the
        # big conv can run native-padded and a tiny patch conv fix the head. Measured net -11us --
    ...
    def forward_decode(self, x):
        ...
        # recurrent_decode_wh.py) -- rms_norm reduces the last dim (V==Dv)
        if self._decode_tile_opt:
