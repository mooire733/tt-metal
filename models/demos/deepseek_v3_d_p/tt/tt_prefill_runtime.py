...
class TtPrefillRuntime:
    ...
    def _prepare_llama4_scale_offsets(self, chunk: int) -> None:
        ...
        One shared set for all layers -- the traced path already shares one
        ChunkMetadata.llama4_scale and the contents are layer-invariant -- but linear in max_seq_len,
        and the cost per offset scales with heads_local, i.e. INVERSELY with TP:
        ...
        Measured after the fix: 5.111 s at 102,400, against 5.745 s pre-temperature.
        """
        self._llama4_scale_by_offset = {}
        if self._trace_metadata.llama4_scale is None:
            return
        # Floor would silently omit the last offset and the request would then die mid-run on the
        # off-grid raise in prefill_chunk. Fail at compile() instead, where it is actionable.
    ...
    def prefill_chunk(
        ...
        if self.config.use_trace:
            ...
            # The three scalars are consumed on-device from metadata_msg, which is the point of this
            # path: the host ints below are never read for the softmax bound.
            if self._trace_metadata.llama4_scale is not None:
                # Mistral's query temperature. The caller still passes actual_start, so it can SELECT
                # a pre-built buffer even though it cannot build one inside a capture. The copy and
                # the replay are both on cq 0, so it is ordered ahead of every layer's read.
                #
                # Raises rather than asserts, unlike most guards in this tree: `python -O` strips
                # asserts, and stripping these does not crash -- it re-enables the silent
    ...
    def release_trace(self) -> None:
        ...
        self._trace_d2h_service = None
        # Sized by max_seq_len, so this is the largest thing the traced path holds (250 MiB/device at
        # 102,400 on an (8,1) stage). Nothing survives the capture it was built for.
