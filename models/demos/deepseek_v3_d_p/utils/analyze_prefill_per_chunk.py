...
A throughput cell reports one median, which at long context hides the thing that matters: cost per
chunk grows with KV depth, because chunk c attends to KV[0 : c*chunk].

The runner logs one line per chunk:

    [pp rank 0] CHUNK_START c=3 compute_start=1789521874.29 slot=0 [15360,20480)

so a chunk's cost is the interval to the NEXT chunk's start on the same rank. Two consequences:

  * It is an INTERVAL, not isolated device time -- it includes host work between two issues. It is
    the quantity analyze_prefill_throughput.py medians, so the two agree by construction.
  * N chunks give N-1 intervals; the last has no successor. A runner that also logs
    `CHUNK_END c=<c> compute_end=<t>` closes that gap with real device-completion times; this
    script uses those when present and reports which source it used.

ON A PIPELINE THESE ARE TWO DIFFERENT QUANTITIES and conflating them is the trap this script fell
into once: the last rank's start-to-start gap is the PERIOD (how often a chunk retires), not how
long a chunk took. Measured, a chunk retired 81 ms after the previous one having spent 1,317 ms
crossing four stages -- so the period made PP=4 look faster than single rank on chunk 1 when it is
really ~10x slower. For P>1 both are reported:
...
rank stays saturated. On PP read the LAST rank: it retires chunks, so its interval is the period.

See also: analyze_prefill_kv_ramp.py, which answers the same question from a Tracy ops capture and
attributes the growth to specific ops (RingJointSDPA grows ~5x across a request; MoE is flat).
Better when you need to know WHY; it costs an instrumented run. This needs only a log you have.
"""
