...
TEST_F(BuilderConnectionMappingTest, MeshRouter_ChannelAndConnectionMapping_Consistent) {
    ...
    for (uint32_t vc = 0; vc < num_vcs; ++vc) {
        ...
        if (num_senders > 0) {
            EXPECT_TRUE(conn_mapping.has_targets(vc, 0)) << "VC" << vc << " receiver channel 0 should have targets";
...
TEST_F(BuilderConnectionMappingTest, ZRouter_ChannelAndConnectionMapping_Consistent) {
    ...
    EXPECT_TRUE(conn_mapping.has_targets(1, 0)) << "Z router VC1 receiver channel 0 should have connection targets";
...
TEST_F(BuilderConnectionMappingTest, VariantDetection_MeshRouter) {
    ...
    for (auto dir : mesh_directions) {
        ...
        EXPECT_EQ(variant, RouterVariant::MESH) << "Direction " << static_cast<int>(dir) << " should be MESH variant";
    }
}
...
TEST_F(BuilderConnectionMappingTest, FactoryMethod_AllMeshConfigurations) {
    ...
    for (const auto& [topology, direction, has_z] : configs) {
        ...
        // Basic validation
        EXPECT_GT(mapping.get_total_sender_count(), 0) << "Mapping should have at least one sender channel";

        // Receiver channel 0 should have targets
        EXPECT_TRUE(mapping.has_targets(0, 0)) << "Receiver channel 0 should have targets";
        ...
        if (has_z) {
            ...
            EXPECT_NE(mesh_to_z_it, targets.end()) << "Should have MESH_TO_Z target";
        }
    }
}
...
TEST_F(BuilderConnectionMappingTest, Scenario_FullDevice_4Mesh1Z) {
    ...
    for (auto dir : mesh_dirs) {
        mesh_mappings.push_back(RouterConnectionMapping::for_mesh_router(Topology::Mesh, dir, true));
    }
    ...
    for (const auto& mapping : mesh_mappings) {
        ...
        EXPECT_NE(mesh_to_z_it, targets.end()) << "Mesh router should have MESH_TO_Z target";
    }
...
TEST_F(BuilderConnectionMappingTest, Scenario_EdgeDevice_2Mesh1Z) {
    ...
    auto z_mapping = RouterConnectionMapping::for_z_router();
    EXPECT_EQ(z_mapping.get_total_sender_count(), 4) << "Z router should have intent for all 4 directions";
}
