...
protected:
    void SetUp() override { registry_ = std::make_shared<ConnectionRegistry>(); }
     ...
     */
    RouterArchetype create_z_router_archetype(FabricNodeId node_id, uint8_t eth_chan = 0) {
...
TEST_F(RouterArchetypesTest, CreateZRouterArchetype) {
    auto router = create_z_router_archetype(FabricNodeId(MeshId{0}, 100));
...
TEST_F(RouterArchetypesTest, NonZToNonZ_1D_Bidirectional) {
    ...
    auto north_router =
        create_mesh_router_archetype(Topology::Linear, RoutingDirection::N, false, FabricNodeId(MeshId{0}, 0));

    auto south_router =
        create_mesh_router_archetype(Topology::Linear, RoutingDirection::S, false, FabricNodeId(MeshId{0}, 1));
    ...
    record_archetype_connection(
        north_router, 0, 0, south_router, 0, north_targets[0].target_sender_channel, ConnectionType::INTRA_MESH);
    ...
    record_archetype_connection(
        south_router, 0, 0, north_router, 0, south_targets[0].target_sender_channel, ConnectionType::INTRA_MESH);
...
TEST_F(RouterArchetypesTest, NonZToNonZ_2D_MultipleDirections) {
    ...
    auto south_it = std::find_if(targets.begin(), targets.end(), [](const ConnectionTarget& t) {
        return t.target_direction == RoutingDirection::S;
    });
    ...
    record_archetype_connection(
        ...
        routers[2],
        0,
        south_it->target_sender_channel,  // SOUTH router
    ...
    auto east_it = std::find_if(targets.begin(), targets.end(), [](const ConnectionTarget& t) {
        return t.target_direction == RoutingDirection::E;
    });
    auto west_it = std::find_if(targets.begin(), targets.end(), [](const ConnectionTarget& t) {
        return t.target_direction == RoutingDirection::W;
    });
    ...
    record_archetype_connection(
        north_router, 0, 0, routers[1], 0, east_it->target_sender_channel, ConnectionType::INTRA_MESH);
    record_archetype_connection(
        north_router, 0, 0, routers[3], 0, west_it->target_sender_channel, ConnectionType::INTRA_MESH);
...
TEST_F(RouterArchetypesTest, NonZToZ_SingleMeshRouter) {
    ...
    // Create Z router
    auto z_router = create_z_router_archetype(FabricNodeId(MeshId{0}, 100));
    ...
    record_archetype_connection(
        mesh_router, 0, 0, z_router, 0, z_target_it->target_sender_channel, ConnectionType::MESH_TO_Z);
...
TEST_F(RouterArchetypesTest, NonZToZ_FourMeshRouters_VC1_Connections) {
    ...
    for (size_t i = 0; i < 4; ++i) {
        record_archetype_connection(
            mesh_routers[i],
            1,
            0,  // Mesh VC1, sender channel 0
            z_router,
            1,
            static_cast<uint32_t>(i),  // Z VC1, receiver channel i
...
TEST_F(RouterArchetypesTest, NonZToZ_FiveMeshRouters_VC1_ShouldFail) {
    ...
    for (size_t i = 0; i < 5; ++i) {
        mesh_routers.push_back(
            create_mesh_router_archetype(Topology::Mesh, RoutingDirection::N, true, FabricNodeId(MeshId{0}, i)));
    }
    ...
    for (size_t i = 0; i < 4; ++i) {
        record_archetype_connection(
            mesh_routers[i], 1, 0, z_router, 1, static_cast<uint32_t>(i), ConnectionType::MESH_TO_Z);
    }
...
TEST_F(RouterArchetypesTest, ZToNonZ_SingleMeshRouter) {
    ...
    // Create mesh router
    auto mesh_router =
        create_mesh_router_archetype(Topology::Mesh, RoutingDirection::N, true, FabricNodeId(MeshId{0}, 0));

    // Z router receiver channel 0 has 4 targets, find NORTH
    auto targets = z_router.connection_mapping.get_downstream_targets(1, 0);
    ASSERT_EQ(targets.size(), 4);

    auto north_it = std::find_if(targets.begin(), targets.end(), [](const ConnectionTarget& t) {
        return t.target_direction == RoutingDirection::N;
    });
    ...
    record_archetype_connection(
        z_router, 1, 0, mesh_router, 1, north_it->target_sender_channel, ConnectionType::Z_TO_MESH);
...
TEST_F(RouterArchetypesTest, ZToNonZ_FourMeshRouters_AllDirections) {
    ...
    for (size_t i = 0; i < 4; ++i) {
        mesh_routers.push_back(
            create_mesh_router_archetype(Topology::Mesh, directions[i], true, FabricNodeId(MeshId{0}, i)));
    }
    ...
    for (size_t i = 0; i < z_targets.size(); ++i) {
        ...
        record_archetype_connection(
            z_router, 1, 0, mesh_routers[i], 1, z_targets[i].target_sender_channel, ConnectionType::Z_TO_MESH);
    }
...
TEST_F(RouterArchetypesTest, ZToNonZ_EdgeDevice_TwoMeshRouters) {
    ...
    auto east_router =
        create_mesh_router_archetype(Topology::Mesh, RoutingDirection::E, true, FabricNodeId(MeshId{0}, 1));

    // Z router has intent for all 4 directions, but only 2 exist
    std::map<RoutingDirection, RouterArchetype*> existing_routers = {
        {RoutingDirection::N, &north_router}, {RoutingDirection::E, &east_router}};
    ...
    for (const auto& target : z_targets) {
        ...
        if (existing_routers.contains(target_dir)) {
            record_archetype_connection(
                ...
                1,
                0,
                *existing_routers[target_dir],
                1,
                target.target_sender_channel,
...
TEST_F(RouterArchetypesTest, FullDevice_4MeshRouters_1ZRouter_AllConnections) {
    ...
    for (const auto& mesh_router : mesh_routers) {
        ...
        auto mesh_to_z_conns = std::count_if(mesh_out.begin(), mesh_out.end(), [](const auto& c) {
            return c.connection_type == ConnectionType::MESH_TO_Z;
        });
        EXPECT_EQ(mesh_to_z_conns, 1);

        // Each mesh router receives from Z (1 Z_TO_MESH)
        auto z_to_mesh_conns = std::count_if(mesh_in.begin(), mesh_in.end(), [](const auto& c) {
            return c.connection_type == ConnectionType::Z_TO_MESH;
        });
