...
TEST_F(RouterConnectionMappingTest, MeshRouter_1D_WithZ_TwoConnections) {
    ...
    auto intra_mesh_it = std::find_if(
        targets.begin(), targets.end(), [](const ConnectionTarget& t) { return t.type == ConnectionType::INTRA_MESH; });
    ...
    auto mesh_to_z_it = std::find_if(
        targets.begin(), targets.end(), [](const ConnectionTarget& t) { return t.type == ConnectionType::MESH_TO_Z; });
...
TEST_F(RouterConnectionMappingTest, MeshRouter_1D_AllDirections_OppositeMapping) {
    ...
    for (const auto& [my_dir, expected_opposite] : direction_pairs) {
        auto mapping = RouterConnectionMapping::for_mesh_router(Topology::Linear, my_dir, false);
...
TEST_F(RouterConnectionMappingTest, MeshRouter_2D_NoZ_ThreeConnections) {
    auto mapping = RouterConnectionMapping::for_mesh_router(Topology::Mesh, RoutingDirection::N, false);
...
TEST_F(RouterConnectionMappingTest, MeshRouter_2D_WithZ_FourConnections) {
    auto mapping = RouterConnectionMapping::for_mesh_router(Topology::Mesh, RoutingDirection::E,
                                                            true);  // Has Z router
...
TEST_F(RouterConnectionMappingTest, MeshRouter_2D_EastRouter_CorrectDirections) {
    auto mapping = RouterConnectionMapping::for_mesh_router(Topology::Mesh, RoutingDirection::E, false);
...
TEST_F(RouterConnectionMappingTest, Scenario_2DMesh_WithZ_FullDevice) {
    ...
    auto north_mapping = RouterConnectionMapping::for_mesh_router(Topology::Mesh, RoutingDirection::N, true);
    auto east_mapping = RouterConnectionMapping::for_mesh_router(Topology::Mesh, RoutingDirection::E, true);
    auto south_mapping = RouterConnectionMapping::for_mesh_router(Topology::Mesh, RoutingDirection::S, true);
    auto west_mapping = RouterConnectionMapping::for_mesh_router(Topology::Mesh, RoutingDirection::W, true);
...
TEST_F(RouterConnectionMappingTest, Scenario_1DMesh_NoZ_Bidirectional) {
    ...
    auto north_mapping = RouterConnectionMapping::for_mesh_router(Topology::Linear, RoutingDirection::N, false);
    auto south_mapping = RouterConnectionMapping::for_mesh_router(Topology::Linear, RoutingDirection::S, false);
...
TEST_F(RouterConnectionMappingTest, MeshRouter_InvalidSenderChannel_ReturnsEmpty) {
    ...
    for (uint32_t ch = 1; ch < 10; ++ch) {
        ...
        EXPECT_TRUE(ch_targets.empty()) << "Mesh router VC0 receiver channel " << ch
                                        << " should have no targets (doesn't exist)";
