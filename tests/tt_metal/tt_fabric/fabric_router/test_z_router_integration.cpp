...
TEST_F(ZRouterIntegrationTest, FullDevice_4Mesh1Z_BidirectionalConnectivity) {
    ...
    for (size_t i = 0; i < 4; ++i) {
        record_connection(
            ...
            0,
            builder_config::num_sender_channels_2d_mesh,  // VC0, MESH_TO_Z channel
            device0,
            RoutingDirection::Z,
            4,
            0,
            0,  // Z VC0 receiver 0 (SAME for all - multi-target)
...
TEST_F(ZRouterIntegrationTest, ConnectionMapping_MeshWithZ_HasMeshToZTarget) {
    ...
    auto mapping = RouterConnectionMapping::for_mesh_router(Topology::Mesh, RoutingDirection::N, true);  // has_z = true
...
TEST_F(ZRouterIntegrationTest, EdgeDevice_2Mesh1Z_MinimalConfiguration) {
    ...
    for (size_t i = 0; i < 2; ++i) {
        record_connection(
            ...
            4,
            1,
            static_cast<uint32_t>(i),  // Z uses senders 0 (N) and 1 (E)
            device0,
            mesh_dirs[i],
            static_cast<uint8_t>(i),
            1,
...
TEST_F(ZRouterIntegrationTest, EdgeDevice_3Mesh1Z_PartialConfiguration) {
    ...
    std::vector<RoutingDirection> mesh_dirs = {RoutingDirection::N, RoutingDirection::E, RoutingDirection::S};
...
TEST_F(ZRouterIntegrationTest, NoZRouter_NoMeshToZConnections) {
    ...
    record_connection(
        device0, RoutingDirection::N, 0, 0, 1, device0, RoutingDirection::S, 2, 0, 1, ConnectionType::INTRA_MESH);

    record_connection(
        device0, RoutingDirection::E, 1, 0, 1, device0, RoutingDirection::W, 3, 0, 1, ConnectionType::INTRA_MESH);
...
TEST_F(ZRouterIntegrationTest, ConnectionMapping_MeshWithoutZ_NoMeshToZ) {
    ...
    auto mapping =
        RouterConnectionMapping::for_mesh_router(Topology::Mesh, RoutingDirection::N, false);  // has_z = false
...
TEST_F(ZRouterIntegrationTest, ConnectionMapping_ZRouter_VC0_CannotBeDownstreamTarget) {
    ...
    auto mesh_mapping =
        RouterConnectionMapping::for_mesh_router(Topology::Mesh, RoutingDirection::N, true);  // has_z = true

    auto mesh_targets = mesh_mapping.get_downstream_targets(0, 0);
    ASSERT_EQ(mesh_targets.size(), 4);  // 3 INTRA_MESH + 1 MESH_TO_Z

    auto mesh_to_z_it = std::find_if(mesh_targets.begin(), mesh_targets.end(), [](const ConnectionTarget& t) {
...
TEST_F(ZRouterIntegrationTest, ZRouter_VC0_NoDownstreamReceiverConnections) {
    ...
    record_connection(
        ...
        0,
        0,
        builder_config::num_sender_channels_2d_mesh,  // mesh VC0 MESH_TO_Z channel
        device0,
        RoutingDirection::Z,
        4,
        0,
        0,  // Z VC0 receiver 0 (CORRECT)
        ConnectionType::MESH_TO_Z);
...
TEST_F(ZRouterIntegrationTest, AllTopologies_MeshMappingWorks) {
    ...
    std::vector<Topology> topologies = {Topology::Linear, Topology::Ring, Topology::Mesh, Topology::Torus};

    for (auto topology : topologies) {
        auto mapping = RouterConnectionMapping::for_mesh_router(topology, RoutingDirection::N, false);

        // Should be able to create mapping without errors
        EXPECT_NO_THROW({ auto targets = mapping.get_downstream_targets(0, 1); })
...
TEST_F(ZRouterIntegrationTest, LinearTopology_NoMeshToZ) {
    ...
    auto mapping =
        RouterConnectionMapping::for_mesh_router(Topology::Linear, RoutingDirection::N, true);  // has_z = true
...
TEST_F(ZRouterIntegrationTest, ZRouter_VC1_ExceedsMaxMeshRouters) {
    ...
    for (size_t i = 0; i < 5; ++i) {
        record_connection(
            ...
            4,
            0,
            0,  // All target same receiver (multi-target)
            ConnectionType::MESH_TO_Z);
    }
