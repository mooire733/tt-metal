...
def test_reshape_nd_input_last_dim_change_is_not_a_view(device):
    ...
    # 2 shards over 1 core, so normalization is rejected and the config stays genuinely ND_SHARDED.
    nd_shard_spec = ttnn.NdShardSpec(
        shard_shape=ttnn.Shape([16, 32]), grid=grid, orientation=ttnn.ShardOrientation.ROW_MAJOR
    )

    torch.manual_seed(0)
    torch_input = torch.randn([30, 32], dtype=torch.bfloat16)  # padded to [32, 32] by TILE
    torch_output = torch_input.reshape([32, 30])
    ...
    assert list(tt_input.padded_shape) == [32, 32], f"got {list(tt_input.padded_shape)}"

    # Padded shapes match (both [32, 32]) and the volume is unchanged (960), so only the last-dim
    # check keeps this off the view path.
    tt_output = ttnn.reshape(tt_input, [32, 30], [32, 32])

    assert list(tt_output.shape) == [32, 30], f"got {list(tt_output.shape)}"
    actual = ttnn.to_torch(tt_output).to(torch.bfloat16)
    _assert_reshape(torch_output, actual, ttnn.bfloat16)
