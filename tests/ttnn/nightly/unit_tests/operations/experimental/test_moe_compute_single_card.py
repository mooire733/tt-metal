...
def _run_moe_compute_single_card_test(
    ...
    None (the 1x1 FullLocal path) or 0, an axis of extent 1 on every mesh shape used here. With
    an explicit axis the op takes its ordinary fabric path, whose combine has no neighbours on
    that axis and degenerates to the same local combine without consulting the fabric (this
    fixture never enables one).

    On a 1xN mesh with op_cluster_axis=0 every device runs one local combine over the same
...
def test_moe_compute_multi_device_local_axis(mesh_device, mesh_shape, expect_error, ccl_knobs):
    """cluster_axis=0 on a 1x4 mesh names an axis of extent 1: the fabric combine has no
    neighbours there and degenerates to a local combine at every device, on a mesh opened
    without a fabric config. The replicated token set and identical routing metadata go to
    every device, each device combines its own expert shard, every device's partial matches the
    golden rows its experts own, all outputs keep the replicated input topology, and a
    dim-0-sharded input is rejected. With ccl_knobs the Galaxy-style topology / num_links / mux
...
def test_moe_compute_local_axis_rejects_shared_experts(mesh_device, mesh_shape, expect_error):
    """cluster_axis over an axis of extent 1 leaves one partial per device that the caller sums,
    so it has no shared-expert path; the 1x1 cluster_axis=None call keeps shared experts. The
    rejection fires at the op boundary before any fabric lookup."""
    with expect_error(RuntimeError, r"mesh axis of extent 1 runs a local combine.*shared experts"):
