...
_REFERENCE_SHA256 = "f8f4265e5e102d0cb2f2cfe3e9f959fcc27f5abedaaf53248713204f0493f54d"
...
def _declared_derivative(x):
    ...
    abs = np.abs
    exp = np.exp
    return np.broadcast_to(np.asarray(exp((x - abs(x)) / 2), dtype=np.float64), x.shape)
...
def test_celu_bw_bf16_exhaustive(device):
    ...
    result = ttnn.to_torch(
        ttnn.celu_bw(device_gradient, device_input, alpha=1.0, memory_config=ttnn.DRAM_MEMORY_CONFIG)[0]
    ).to(torch.bfloat16)
