...
"""All BF16 inputs; assert finite accuracy and evidenced raw special outputs."""
...
def _assert_finite_math(raw_words, result_words, reference_values, expected_specials):
    """Check finite accuracy and every evidenced raw exceptional class."""
    ...
    raw_classes = _raw_classes(raw_words)
    for raw_class, expected_word in expected_specials.items():
        observed = result_words[raw_classes == raw_class]
        assert observed.size
        assert np.all(observed == np.uint16(expected_word)), (
            raw_class,
...
def test_erfc_bf16_exhaustive(device):
    ...
    if is_blackhole():
        positive_nonfinite = 0x2C4F
    else:
        assert is_wormhole_b0()
        positive_nonfinite = 0x2C4E
    expected_specials = {
        "pos_zero": 0x3F80,
        "neg_zero": 0x3F80,
        "pos_subnormal": 0x3F80,
        "neg_subnormal": 0x3F80,
        "pos_inf": positive_nonfinite,
        "neg_inf": 0x4000,
        "pos_nan": positive_nonfinite,
        "neg_nan": 0x7F80,
    }
    _assert_finite_math(raw, result_words, reference_values, expected_specials)
