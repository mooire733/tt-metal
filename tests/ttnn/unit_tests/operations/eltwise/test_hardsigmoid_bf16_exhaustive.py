...
"""All BF16 inputs; finite mathematical accuracy. Paired qualification owns specials."""
...
def _declared_piece_1(x):
    ...
    return np.broadcast_to(np.asarray(x / 6 + 0.5, dtype=np.float64), x.shape)
...
def test_hardsigmoid_bf16_exhaustive(device):
    ...
    result = ttnn.to_torch(ttnn.hardsigmoid(device_input)).to(torch.bfloat16)
