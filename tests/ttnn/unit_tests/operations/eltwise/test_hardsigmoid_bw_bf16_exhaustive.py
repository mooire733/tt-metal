...
def test_hardsigmoid_bw_bf16_exhaustive(device):
    ...
    declared_special_mismatches = 0

    def run(activation, gradient):
        nonlocal declared_special_mismatches
        ...
        # Retain every finite input, including zeros and subnormals.
        finite = ((activation & np.uint16(0x7F80)) != 0x7F80) & ((gradient & np.uint16(0x7F80)) != 0x7F80)
        equal_zero = ((actual & np.uint16(0x7FFF)) == 0) & ((expected & np.uint16(0x7FFF)) == 0)
        assert np.all(((actual == expected) | equal_zero)[finite])
        declared_special_mismatches += int(np.count_nonzero((actual != expected) & ~finite))
