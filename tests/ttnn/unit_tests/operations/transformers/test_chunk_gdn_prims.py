...
def _clean_env(monkeypatch):
    ...
    monkeypatch.delenv("QWEN_GDN_TINV", raising=False)  # WY-inverse method: the Horner reference unless a test sets it
...
# ---------------------------------------------------------------------------
# (4) the WY-inverse methods (QWEN_GDN_TINV) through the prep prim
# ---------------------------------------------------------------------------
...
# (Blackhole p300c, 2026-09-22; measured in the comment). Every method shares the error of the device's
# own N (tf32-class matmul operands, SFPU exp in L_mask), which is the ~1e-3 floor; the bf16 staging of
# the SFPU-bf16 variant adds its own rounding on top, visible in the hard regime.
_TINV_BOUNDS = {
    ("horner", "typical"): 2.1e-3,  # 1.04e-3
    ("horner", "hard"): 7.3e-3,  # 3.65e-3
    ("horner", "adversarial"): 3.4e-3,  # 1.67e-3
    ("sfpu_bf16", "typical"): 1.8e-3,  # 0.90e-3
    ("sfpu_bf16", "hard"): 1.5e-2,  # 7.51e-3
    ("sfpu_bf16", "adversarial"): 5.3e-3,  # 2.63e-3
    ("sfpu_fp32", "typical"): 2.0e-3,  # 0.98e-3
    ("sfpu_fp32", "hard"): 6.1e-3,  # 3.04e-3
    ("sfpu_fp32", "adversarial"): 3.4e-3,  # 1.67e-3
}


@pytest.mark.parametrize("regime", ["typical", "hard", "adversarial"])
def test_prep_tinv_methods(device, monkeypatch, regime):
    """The three WY-inverse methods on the same device-built N: each T_inv within its bound of the fp64
    inverse (finite, even where powers of N reach ~1e8), the fp32-L SFPU solve no less accurate than the
    Horner reference, and the six other prep outputs bit-identical across methods — the inverse is the
    only thing the method changes."""
    _clean_env(monkeypatch)
    ...
    for method in ("horner", "sfpu_bf16", "sfpu_fp32"):
        ...
        got = [ttnn.to_torch(o) for o in ttnn.transformer.chunk_gdn_prep(*tensors, *const_tiles, chunk_size=CHUNK)]
    ...
    assert errs["sfpu_fp32"] <= 1.05 * errs["horner"], f"{regime}: fp32-L SFPU solve less accurate than Horner: {errs}"
