...
constexpr std::uint32_t kClampHold = ADDR_MOD_7;
constexpr std::uint32_t kClampAdvance = ADDR_MOD_6;
#else
...
inline void init_clamped_affine() {
    ...
    if constexpr (Config::kRoute == 4u) {
        ...
        // one-time config-register RMW; the direct LLK call is pack-thread-gated,
        // so this initializer must be invoked at kernel scope
...
#ifdef TRISC_PACK
            llk_pack_relu_config(ckernel::ReluConfig::max_threshold(Config::kPackReluThresholdBits));
#endif
        } else {
#ifdef TRISC_PACK
            llk_pack_relu_config(ckernel::ReluConfig::zero());
#endif
        }
    }
}
...
#ifdef TRISC_PACK
        llk_pack_relu_config(ckernel::ReluConfig::none());
#endif
    }
}
...
inline void calculate_clamped_affine() {
    if constexpr (Config::kRoute == 0u) {
        ...
        ckernel::sfpu::_relu_min_<sfpi::vFloat, false, Iterations, std::uint32_t>(0u);
