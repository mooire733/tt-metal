...
// Which ops the sanitizer models, and the Operation each is modelled as. Adding an op is one
// specialization here; everything else keeps the unsupported() fence.
...
struct sfpu_operation<SfpuType::sqrt> {
    ...
    using type = llk::san::OperationSfpuSqrt;
};
...
SAN_SFPU_TAG(calculate_situ_glu, unused);
SAN_SFPU_TAG(calculate_snake_beta, snake_beta);
SAN_SFPU_TAG(calculate_softcap, unused);
