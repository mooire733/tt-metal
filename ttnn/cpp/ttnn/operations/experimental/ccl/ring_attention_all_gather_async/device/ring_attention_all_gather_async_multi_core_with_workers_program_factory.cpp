...
void ring_attention_neighbor_halo_exchange_helper(
    ...
    for (uint32_t link = 0; link < num_links; ++link) {
        ...
        // the whole halo. The receiver's rendezvous core mirrors the sender's, as the one-hop
        // protocol already relied on (both sides lay out workers identically).
        ...
        for (uint32_t input = 0; input < num_inputs; ++input) {
            ...
            // Where this hop's block starts in the receiver's compact buffer. The writer sends to
            // `dest_page = src_page - input_origin + output_origin`, so a one-hop halo (origin 0)
            // keeps landing at row 0 exactly as before.
        ...
        // Link hand-off, read by the writer straight after the fabric args. Zero/zero whenever this
        // hop owns its link outright, which is every one-hop halo and every halo with hops <= links.
