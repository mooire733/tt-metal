        ...
        for (uint32_t local_h = 0; local_h < stride_height; ++local_h) {
            ...
            for (uint32_t w_tile = 0; w_tile < tiles_per_width_dim; ++w_tile) {
                ...
                for (uint32_t local_w = 0; local_w < width_limit; ++local_w) {
                    ...
                    // guaranteed_16B_aligned=true: enforced upstream by the c_bytes % 16 gate, so
                    // no per-pixel misalign fallback (copy_via_memmove skips the WH/BH drain).
                    tt_memmove<true, true, false, c_bytes>(
                        noc,
