...
#include "ttnn/operations/experimental/quasar/interleaved_to_sharded/device/interleaved_to_sharded_op.hpp"
...
bool can_use_sharded_to_interleaved(
    ...
    auto [valid, msg] = ttnn::prim::qsr::ShardedToInterleavedDeviceOperation::validate_inputs(
        ttnn::prim::qsr::ShardedToInterleavedParams{output_mem_config, resolved_dtype},
        ttnn::prim::qsr::ShardedToInterleavedInputs{input_tensor, output_tensor});
    if (!valid) {
        return false;
    }
...
bool can_use_interleaved_to_sharded(
    ...
    const std::optional<Tensor>& output_tensor) {
    DataType resolved_dtype = output_dtype.value_or(input_tensor.dtype());
    auto [valid, msg] = ttnn::prim::qsr::InterleavedToShardedDeviceOperation::validate_inputs(
        ttnn::prim::qsr::InterleavedToShardedParams{output_mem_config, resolved_dtype, false},
        ttnn::prim::qsr::InterleavedToShardedInputs{input_tensor, output_tensor});
    if (!valid) {
        return false;
    }
    ...
    if (!output_mem_config.shard_spec().has_value()) {
        auto output_spec = ttnn::prim::qsr::InterleavedToShardedDeviceOperation::compute_output_specs(
            ttnn::prim::qsr::InterleavedToShardedParams{output_mem_config, resolved_dtype, false},
            ttnn::prim::qsr::InterleavedToShardedInputs{input_tensor, output_tensor});
...
bool can_use_reshard(
    ...
    auto [valid, msg] = ttnn::prim::qsr::ReshardDeviceOperation::validate_inputs(
        ttnn::prim::qsr::ReshardParams{output_mem_config}, ttnn::prim::qsr::ReshardInputs{input_tensor, output_tensor});
    if (!valid) {
        return false;
    }
...
Tensor to_memory_config(
        ...
        if (tensor.is_sharded()) {
            ...
            if (!use_reshard_workaround) {
                ...
                if (can_use_reshard(tensor, memory_config, dtype, output_tensor)) {
                    return ttnn::operations::experimental::quasar::reshard(tensor, memory_config, output_tensor);
                }
            ...
            if (can_use_sharded_to_interleaved(
                ...
                Tensor temp = ttnn::prim::qsr::sharded_to_interleaved(
    ...
    if (tensor.is_sharded() && can_use_sharded_to_interleaved(tensor, memory_config, dtype, output_tensor)) {
        return ttnn::prim::qsr::sharded_to_interleaved(
