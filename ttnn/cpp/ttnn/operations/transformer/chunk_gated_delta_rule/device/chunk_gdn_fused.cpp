...
void ChunkGdnFusedOperation::validate_on_program_cache_miss(
    ...
    // Per-(head, slot) credit words live in one 4 KB tile of the u/mask CB (design D6 v0.3).
    TT_FATAL(
    ...
    if (attrs.placement == 0) {
        ...
        // Row-local (design D9 v0.3): L = NV+NP cores per head in one row; heads beyond grid.y go to the
...
// them (BH=4), 34 us with >= 84 (BH=12); linear in between (design §5 v0.3, §10e).
...
float t_step_us(uint32_t Vtl) {
    switch (Vtl) {
        case 1: return 3.5f;    // receiver period after the Phase 3 scan-step batching (§10e): 3.43 compute
...
FusedPlacement fused_placement(
    ...
    // ---- Placement (design D9; test_chunk_gdn_fused_geometry.py checks it against a Python oracle) ----
...
FusedGeometryChoice choose_fused_geometry(
    ...
    if (!have) {  // no row-local layout: the v0.2 row-major fallback (optimistic — ignores link sharing)
        for (uint32_t nv : {1u, 2u, 4u, 8u}) {
