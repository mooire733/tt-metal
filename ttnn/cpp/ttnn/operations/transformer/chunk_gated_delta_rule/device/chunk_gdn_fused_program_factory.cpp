...
//
// Geometry (design D3/D9; computed by fused_placement() in chunk_gdn_fused.cpp, which the host-side
...
tt::tt_metal::ProgramDescriptor ChunkGdnFusedProgramFactory::create_descriptor(
    ...
    // Placement (design D9) is a pure function of (grid, BH, NV, NP, placement), shared with the
    ...
    // BH x nbuf leading words are the producer-side credit counters credit[h][slot] (design D6 v0.3).
    ...
    //                   flight (design D6 v0.3). Consecutive ids => consecutive L1 words, so the kernels
    ...
    for (uint32_t h = 0; h < BH; h++) {
        ...
        // which the multicast tolerates — Phase 1's 1xNV row rectangles crossed that gap all along.
