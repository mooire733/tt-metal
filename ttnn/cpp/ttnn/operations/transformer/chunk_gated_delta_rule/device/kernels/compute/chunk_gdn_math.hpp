...
// out exactly as the pre-F0 kernel's `if constexpr` did, or the Ct==2 prep program overflows the
...
inline void scan_step(const GdnScanCbs& cb, uint32_t cur_S, uint32_t dst) {
    ...
    // for the whole step: skip the per-call reconfigs (design §10e; the producer does the same under
