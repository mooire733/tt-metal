...
// element-wise from its L1 tile and splatted across the 32 SFPU lanes as an SFPLOADI immediate; the RHS
// rows and the solution live in DEST (fp32 when fp32_dest_acc_en, as in the GDN kernels).
//
// Two L-read variants (template L_FP32):
//   false: negN must be staged as a bf16 tile first — one SFPLOADI (FLOATB) per element. The staging
//          rounds negN to bf16, which is the dominant error term.
//   true : negN is read straight from its fp32 tile — two SFPLOADIs (UPPER, LOWER) per element, no
//          staging copy, no rounding of L.
//
...
// Splat one L element into LREG7 across all lanes.
template <bool L_FP32>
...
template <bool L_FP32, typename Elem>
inline void gdn_tinv_apply_prev_col(volatile tt_l1_ptr Elem* l_col, std::uint32_t x_addr) {
...
inline void gdn_tinv_trisolve(std::uint32_t dst_in, std::uint32_t dst_out, std::uint32_t l1_base) {
    using Elem = std::conditional_t<L_FP32, std::uint32_t, std::uint16_t>;
    constexpr std::uint32_t dst_tile_size = 64;  // DEST rows per tile
    volatile tt_l1_ptr Elem* const tile = reinterpret_cast<volatile tt_l1_ptr Elem*>(l1_base);
    ...
    for (std::uint32_t chunk = 0; chunk < 8; chunk++) {
        ...
        if (row0 > 0) {
            ...
            volatile tt_l1_ptr Elem* l_face0 = tile + gdn_tinv_elem(row0, 0);
            for (std::uint32_t col = 0; col < nface0; col++) {
                gdn_tinv_apply_prev_col<L_FP32>(l_face0 + col, out_base + kGdnTinvRowOff[col]);
            }
            if (row0 > 16u) {
                volatile tt_l1_ptr Elem* l_face1 = tile + gdn_tinv_elem(row0, 16);
                for (std::uint32_t col = 16; col < row0; col++) {
                    gdn_tinv_apply_prev_col<L_FP32>(l_face1 + (col - 16u), out_base + kGdnTinvRowOff[col]);
                }
            }
        }
        ...
        {
            volatile tt_l1_ptr Elem* l_row = tile + gdn_tinv_elem(row0 + 1, row0);
            gdn_tinv_load_l<L_FP32>(l_row[0]);
        ...
        {
            volatile tt_l1_ptr Elem* l_row = tile + gdn_tinv_elem(row0 + 2, row0);
        ...
        {
            volatile tt_l1_ptr Elem* l_row = tile + gdn_tinv_elem(row0 + 3, row0);
            const std::uint32_t b0 = l_row[0];
            gdn_tinv_load_l<L_FP32>(b0);
            const std::uint32_t b1 = l_row[1];
...
// Solve (I - negN) X = DST[idst_in] into DST[idst_out]. negN is tile `l_tile_idx` of cb_l (front-waited):
// bf16 when L_FP32 is false, fp32 when true. DST must be acquired; call gdn_tinv_trisolve_tile_init() first.
template <bool L_FP32>
ALWI void gdn_tinv_trisolve_tile(CircularBuffer& cb_l, uint32_t l_tile_idx, uint32_t idst_in, uint32_t idst_out) {
    ...
    MATH((sfpu::gdn_tinv_trisolve<L_FP32>(idst_in, idst_out, l1_base)));
