...
void apply_ring_joint_scalar_runtime_args(
    ...
    // Derived through the layout, like every other hop count in this file: the relocation loop below
    // indexes kernels as kNeighborHaloReaderKernelIndex + 2 * hop, so a second, independently
    // written derivation that disagreed would patch the wrong kernel's args without erroring.
    const uint32_t neighbor_halo_hop_count =
        uses_neighbor_halo ? ring_joint::build_chunked_sliding_halo_layout(
    ...
    if (patch_indexed_kv_cache) {
        ...
        if (uses_neighbor_halo) {
            for (uint32_t hop = 0; hop < neighbor_halo_hop_count; ++hop) {
                patch_reader_batch_base(
                    kNeighborHaloReaderKernelIndex + 2 * hop,
    ...
    if (args.has_sliding_window() && (patch_indexed_kv_cache || patch_kv_pad_rotation)) {
        ...
        // One reader/writer pair per hop, appended in hop order after the three SDPA kernels.
        const uint32_t runtime_hop_count = runtime_chunked_sliding_layout.remote_hop_count();
        for (uint32_t hop = 1; hop <= runtime_hop_count; ++hop) {
            const uint32_t runtime_tail_start_Ht =
                runtime_chunked_sliding_layout.send_tail_start_tile(ring_write_plan.transport_rank, hop);
...
tt::tt_metal::ProgramDescriptor build_ring_joint_sdpa_program_descriptor(
    ...
    */
    ...
    std::vector<uint32_t> reader_compile_time_args = {
        ...
        circular_kv_slab_count,
        // Slots 45-46: logical-length transport. The reader NoC-reads the live values, re-derives
        // logical_nt / logical_lt / the ring masks, and fans them to compute via cb_kv_pad_derived.
        // Tensor accessors start at slot 47.
    ...
    std::vector<uint32_t> writer_compile_time_args = {
        ...
        rank_mapping.mesh_rows,
        rank_mapping.mesh_cols,
        // Slots 42-43: logical-length transport. The writer is a dataflow kernel, so it NoC-reads the live
        // values itself rather than sharing the reader's L1 mailbox. Output accessors start at slot 44.
        static_cast<uint32_t>(has_logical_n_tensor),
        static_cast<uint32_t>(has_logical_l_tensor),
    };
    ...
    std::vector<uint32_t> compute_compile_time_args = {
        ...
        circular_kv_slab_count,
        // Slots 59-60: logical-length transport. Compute cannot NoC-read DRAM, so it takes the live values
        // from cb_kv_pad_derived, which the reader fills. CB block starts at 61.
        static_cast<uint32_t>(has_logical_n_tensor),
        static_cast<uint32_t>(has_logical_l_tensor)};
    ...
    if (has_sliding_window) {
        ...
        // covered by hop_count exchanges: hop d ships the tail of the slab d positions back into its
        // own block of the receiver's compact buffer. Every hop is an independent fabric unicast, so
        // they all fly in parallel; the SDPA reader waits for all of them before its K loop.
        // The hop that lands back on this device (only possible when the halo spans the whole ring)
        // is a local cache read, so it gets no exchange. The total hop count is validated against the
        // ring size in validate_on_program_cache_miss, which always runs before program creation.
        ...
        // Concurrent exchanges cannot share a fabric link: one worker per (link, direction) owns an
        // EDM channel, and a second worker on the same channel stalls the first mid-send. So a
        // multi-hop halo gives each hop its own link. Aggregate bandwidth is unchanged -- each hop
        // carries 1/h of the same halo.
        ...
        // Derived with the same allocator the helper uses, so the two agree without the helper
        // having to report back. hop_first_cores[h - 1] is hop h's first (and, when h > 1, only)
        // worker.
        ...
        // With more hops than links the hops TIME-SHARE a link instead of each owning one: hop h
        // runs on link (h-1) % span and waits for hop h-span to close its EDM connection first.
        // Sequential reuse of a sender channel is the fabric's own protocol (close() persists the
        // producer cursor, open() adopts it); only concurrent use deadlocks. The serialised hops
        // still overlap their DRAM reads with the predecessor's send, so the cost is the extra
        // fabric time alone.
        const uint32_t halo_link_span = std::min(halo_remote_hops, halo_links_available);
        // One local semaphore, same id on every halo worker, carries the hand-off. Pick an id free
        // on all of them so the fabric's own per-core allocation (which runs later, inside the
        // helper) cannot collide with it.
        uint32_t halo_chain_semaphore_id = 0;
        if (halo_remote_hops > halo_link_span) {
            // Per-core ids are handed out densely from 0, so the largest per-core first-free id is
            // free on every hop core. Assert that rather than assume it: an id already taken
            // somewhere would silently alias another semaphore.
        ...
        for (uint32_t hop = 1; hop <= halo_remote_hops; ++hop) {
            ...
            // backward the other way round instead. At hop 1 this is exactly the device N-1 -> 0 case.
